...
<ScrollView contentContainerStyle={{flexGrow:1,paddingVertical:getResponsiveFontSize(25)}}>
  ...
  {images.length>0 && (
    images.map(((x,key)=>{
        return(  
    <TouchableOpacity style={{marginLeft:'5%',marginTop:'3%'}} onPress={()=>{navigation.navigate("Scan",{scanid:key})}}>
            <Image source={{ uri: x}} style={{ borderWidth:getResponsiveFontSize(2),borderColor:'white',width: getResponsiveFontSize(150), height: getResponsiveFontSize(150) }}  />
    </TouchableOpacity>        
            )
    }))
 )}
